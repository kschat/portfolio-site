...
var WIDGETS = {
	...
	*/
	...
	/*
	* Takes an argument for the side. Returns that sides top value.
	*/
	...
	/*
	* Takes an argument for the key. Determines if the key exists in 
	* either of the widget sides. Returns false if the key doesn't exist,
	* otherwise it returns the widget object.
	*/
	...
	/*
	* Takes a selector, event to trigger, speed for the show animation
	* and callback function as arguments. Finds the widget using the 
	* selector and calls its showWidget function.
	*/
	...
	/*
	* Takes a selector, event to trigger, speed for the hide animation
	* and callback function as arguments. Finds the widget using the 
	* selector and calls its hideWidget function.
	*/
	...
	/*
	* Takes a key and optional side argument. If only the key value
	* is given, it searches through both widget hashmaps, starting with
	* the left one. If the side is given, it searches only that side. Returns
	* the false if the widget isn't found, otherwise returns the widget.
	*/
...
var NavWidget = function(btnID, side, onTop) {
	...
	this.actions = {};
	this.addAction(that.button, "click", function() { that.showWidget({}, 500) } , function() { that.hideWidget({}, 500) });
	this.properties = {};
};
...
/*
* BasicView constructor and prototype
*/
var BasicView = function(element) {
	this.element = element;
}
...
/*
* Make the Overlay class a subclass of BasicView
*/
...
Overlay.prototype = new BasicView();
...
MessageView.prototype = new BasicView();
MessageView.prototype.constructor = MessageView;

MessageView.prototype = {
	show:		function(time, text) {
		if(arguments.length === 1) {
			this.element.text(text).fadeIn(time);
		}
		else {
			this.element.text(text).fadeIn();
		}
	},
	hide:		function(time, text) {
		if(arguments.length === 1) {
			this.element.text(text).fadeOut(time);
		}
		else {
			this.element.text(text).fadeOut();
		}
	}
}
...
var loginWidget = WIDGETS.add('#login', SideEnum.RIGHT, true, {
		...
		signInForm:			{
								...
								disableInput:	function() {
									$(this.id + ' :input').attr('disabled', 'disable');
								},
	...
	var settingsWidget = WIDGETS.add('#settings', SideEnum.RIGHT, false, {username: readCookie('username')});
	var signupWidget = WIDGETS.add('#signup', SideEnum.RIGHT, false, {
		signupMessage:		{
								id:			'#signup-message',
								show:		function(message) {
									$(this.id).text(message).fadeIn(700);
								},
								hide:		function() {
									$(this.id).hide().text('');
								}
...
$(document).ready(function() {
	...
	signupWidget.addAction('#signup-submit', 'click', function(event) {
		...
		$.ajax({
			...
			beforeSend:	function(jqXHR, settings) {
				...
				buttonOverlay.setWidth($('#signup').css('width'));
				buttonOverlay.setHeight($('#signup').css('height'));
				buttonOverlay.show();
			},
	...
	loginWidget.addAction('#login-submit', 'click', function(event) {
		...
		$.ajax({
			...
			beforeSend: 		function(jqXHR, settings) {
				...
				buttonOverlay.setWidth($('#login').css('width'));
				buttonOverlay.setHeight($('#login').css('height'));
				buttonOverlay.show();
			},
			success:			function(data, textStatus, jqXHR) {
				tempOverlay.hide();
	...
	*/
	...
	var notf = new NotificationPanel('#notification1');
	...
	if(page == 'home') {
		var linkQuery = {
			...
			"success":		function(data, textStatus, jqXHR) {
				$('#center-content').html(data).slideDown('slow');
			},
			"error":		function(jqXHR, textStatus, errorThrown) {
				alert('error');
			}
